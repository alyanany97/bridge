...
export default function NeedyHome() {
  ...
  if (!isAdmin && role !== "needy") return null;
  const [tab, setTab] = useState<Tab>("browse");
  const [category, setCategory] = useState<Category>("all");
  const { posts: browsePosts, loading: browseLoading, error } = usePosts("offer");
  const { posts: myPosts, loading: myLoading } = useMyPosts("need");
  ...
  return (
    <PageShell>
          ...
          <>
            ...
            {!browseLoading && !error && filtered.length === 0 && (
              <div className="flex flex-col items-center gap-3 py-16 text-center">
                ...
                <p className="text-sm text-muted-foreground">No offers nearby right now.</p>
              </div>
            )}
            <div className="space-y-3">
              {filtered.map((post) => (
      ...
      <Button
        ...
        onClick={() => navigate("/post/new?kind=need")}
      >
        <Plus size={20} />
        Request help
      </Button>
